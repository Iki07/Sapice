...
//ovaj se deo ponavlja, razmisli o f-ji na all dogs, all charrs, all breeds, needs, users...
    $records="SELECT * FROM characteristics";
        ...
        <div class="main-admin-content">
            <div class="row admin">
                <div class="col-6 mb-5">
                    <h2>Pregled unetih osobina pasa</h2>
                </div><!--col-6-->
                ...
                <div class="col-6 me-auto"> <!--tabela + paginacija-->
                    <table id="chars" class="table table-hover table-responsive">
                        <thead>
                            <tr>
                        ...
                        <?php
                            $query="SELECT * FROM characteristics LIMIT {$limit} OFFSET {$offset}";
                            ...
                            ?>
                                <td scope="col">
                                    <a class="edit-char" href="#" data-id="<?php echo $id?>" data-bs-toggle="tooltip" data-bs-title="Izmeni zapis"><i class="bi bi-pencil"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                        </tbody>
                    ...
                    <button type="submit" class="btn btn-primary mb-3" id="new-char" data-bs-toggle="modal" data-bs-target="#addCharModal">Dodaj novu osobinu</button>
<!--i ovo dole ce da se ponavlja vrv-->
                    <nav aria-label="Paginacija">
                        <ul class="pagination justify-content-center">
                            <li class="page-item">
                                <a href="characteristics.php?page=<?php echo ($current_page - 1); ?>" class="page-link" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                            <?php
                                for ($i = 1; $i <= $pages; $i++) {
                                    echo "<li class='page-item'><a class='page-link' href='characteristics.php?page={$i}'>{$i}</a></li>";
                                }
                            ?>
                            <li class="page-item">
                                <a class="page-link" href="characteristics.php?page=<?php echo ($current_page + 1); ?>" aria-label="Next">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editTextFieldModalLabel">Ažurirajte naziv osobine</h5>
    ...
    <div class="modal fade" id="addCharModal" tabindex="-1" aria-labelledby="addCharModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addCharModalLabel">Unesite novu rasu</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="addCharForm">
                        <div class="mb-3">
                            <label for="char-name" class="form-label">Osobina:</label>
...
//$(document).on('click', '.edit-breed', function() {
    $('.edit-char').click(function() {
        ...
        $('#editTextFieldConfirm').click(function() {
            ...
            updateChar(fieldToEdit.id, newName, newStatus);
        });

        function updateChar(id, name, status) {
            $.ajax({
                url: 'ajax/ajax_update_characteristics.php',
                type: 'POST',
                data: {
                    id: id,
                    name: name,
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(answer.error!=""){
                    ...
                    $("#addCharForm")[0].reset();
                    var addCharModal = bootstrap.Modal.getInstance(document.getElementById('addCharModal'));
                    addCharModal.hide();

                    var newRow=$("<tr>");
                    newRow.append($("<td>").text(answer.char_id));
                    newRow.append($("<td>").addClass("editable-text").attr("data-field", "name").attr("data-id", answer.char_id).text(charName));
                    newRow.append($("<td>").addClass("editable-dropdown").attr("data-field", "status").attr("data-id", answer.char_id).text("aktivan"));

                    $("#chars tbody").append(newRow);
