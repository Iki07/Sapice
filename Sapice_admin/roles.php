        ...
        <div class="main-admin-content">
            <div class="row admin">
                ...
                <div class="col-6 me-auto"> <!--tabela + paginacija-->
                    <table id="roles" class="table table-hover table-responsive">
                        <thead>
                            <tr>
                        ...
                        <?php
                            ...
                            ?>
                                <td scope="col">
                                    <a class="edit-role" href="#" data-id="<?php echo $id?>" data-bs-toggle="tooltip" data-bs-title="Izmeni zapis"><i class="bi bi-pencil"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                        </tbody>
                    ...
                    <button type="submit" class="btn btn-primary" id="new-role" data-bs-toggle="modal" data-bs-target="#addRoleModal">Dodaj novu korisničku ulogu</button>
<!--i ovo dole ce da se ponavlja vrv-->
                    <nav aria-label="Paginacija">
                        <ul class="pagination justify-content-center">
                            <li class="page-item">
                                <a href="roles.php?page=<?php echo ($current_page - 1); ?>" class="page-link" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                            <?php
                                for ($i = 1; $i <= $pages; $i++) {
                                    echo "<li class='page-item'><a class='page-link' href='roles.php?page={$i}'>{$i}</a></li>";
                                }
                            ?>
                            <li class="page-item">
                                <a class="page-link" href="roles.php?page=<?php echo ($current_page + 1); ?>" aria-label="Next">
    ...
    <div class="modal fade" id="addRoleModal" tabindex="-1" aria-labelledby="addRoleModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addRoleModalLabel">Unesite novu rasu</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="addRoleForm">
                        <div class="mb-3">
                            <label for="role-name" class="form-label">Naziv uloge:</label>
...
//$(document).on('click', '.edit-breed', function() {
    $('.edit-role').click(function() {
        ...
        $('#editTextFieldConfirm').click(function() {
            ...
            updateRole(fieldToEdit.id, newName, newStatus);
        });

        function updateRole(id, name, status) {
            $.ajax({
                url: 'ajax/ajax_update_roles.php',
                type: 'POST',
                data: {
                    id: id,
                    name: name,
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(answer.error!=""){
                    ...
                    var newRow=$("<tr>");
                    newRow.append($("<td>").text(answer.role_id));
                    newRow.append($("<td>").addClass("editable-text").attr("data-field", "name").attr("data-id", answer.role_id).text(roleName));
                    newRow.append($("<td>").addClass("editable-dropdown").attr("data-field", "status").attr("data-id", answer.role_id).text("aktivan"));
